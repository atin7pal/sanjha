...
const Carousel = ({ slides }) => {
  ...
  return (
    <div className="relative w-[600px] h-[80vh] mx-auto overflow-hidden flex flex-col justify-center items-start max-sm:w-full max-sm:h-full">
       <h2 className="text-5xl font-bold py-5 themetext max-sm:text-3xl">Our Customers love our lovely food</h2>
      <div className="flex transition-transform duration-500" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
        {Reviews.map((slide, index) => (
          <div key={index} className="w-full flex-shrink-0 flex justify-center items-start flex-col gap-3">
            <p className="text-start mt-2 text-lg font-normal text-gray-600 ">{slide.review}</p>
      ...
      <button onClick={prevSlide} className="rounded-full px-4 py-4 bg-white shadow-lg shadow-gray-200 border border-gray-100"><FaArrowLeft/></button>
      <button onClick={nextSlide} className="rounded-full px-4 py-4 bg-white shadow-lg shadow-gray-200 border border-gray-100"><FaArrowRight/></button>
      </div>
    </div>
  );
};
